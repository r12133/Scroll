...
function logISay(){
    console.clear();
    console.warn('网站设计灵感和图片来源均来自 http://graphicnovel-hybrid4.peugeot.com/start.html');
}
// 设置动画
...
function startScroll(status){
    ...
    control.t = setInterval(() => {
        ...
        var nowSt = document.documentElement.scrollTop;
        if(st == nowSt){
            stopScroll(status)
        }
    },control.speed)
...
// 播放音频
function playAudio(src,isBgMusic,loop){
    var audio = document.createElement('audio');
    if(isBgMusic) {
        audio = document.querySelector('.bg_audio');
        audio.loop = loop || false;
        audio.volume = 0.5;
    }
    audio.src = src || 'http://graphicnovel-hybrid4.peugeot.com/music/BoucleMusique_ZEV.ogg';
    audio.play();
    audio.onended = function(){
        var i = control.audioArr.indexOf(src);
        setTimeout(() => control.audioArr.splice(i,1), 5000);
    }
}
// 初始化
function init() {
...
function completed() {
    ...
    if(control.autoplay){
        setTimeout(() => startScroll(status),control.scrollDelay);
    }
    ...
    sound.onclick = function() {
        control.bgMusicOn = !control.bgMusicOn;
        control.bgMusicOn && openBgMusic(this)
        !control.bgMusicOn && stopBgMusic(this)
    }
    ...
    var page_fourteen_control = [
        ...
        {key: win_h * 20 + 200 + 192 + 30, value: 'bottom: 0px' },
        {key: win_h * 22 + 200 + 192 + 30, value: 'bottom: -100px'},
    ];
...
// 音频检测
function checkAndPlay(src,isBgMusic,loop){
    var i = control.audioArr.indexOf(src);
    if(i < 0) {
        control.audioArr.push(src);
        playAudio(src,isBgMusic,loop);
    }
}
// 全局音频播放
function globalAudioSettings(st){

    // 开始的北京音乐
    if(st >= ( .3 * win_h ) && st <= ( 1.8 * win_h ) ) {
        checkAndPlay('audio/bg_start.ogg',true,true);
    }
    // 踹门
    if(st >= ( 1.3 * win_h ) && st <= ( 1.8 * win_h ) ) {
        checkAndPlay('audio/kick_door.ogg');
    }
    // 小高潮
    if(st >= ( 3.2 * win_h )) {
        checkAndPlay('audio/bg_high.ogg',true,true);
    }
    // 破窗
    if(st >= ( 3.5 * win_h ) && st <= ( 4 * win_h ) ) {
        checkAndPlay('audio/verre.ogg');
    }
    // 猎犬
    if(st >= ( 5.5 * win_h ) && st <= ( 6 * win_h ) ) {
        checkAndPlay('audio/dogs.ogg');
    }
    // 猎犬2
    if(st >= ( 7.5 * win_h ) && st <= ( 8 * win_h ) ) {
        checkAndPlay('audio/dogs_chase.ogg');
    }
    // 车鸣
    if(st >= ( 10.5 * win_h ) && st <= ( 11 * win_h ) ) {
        checkAndPlay('audio/car_drive_far.ogg');
    }
    // 车鸣2
    if(st >= ( 11.8 * win_h ) && st <= ( 13 * win_h ) ) {
        checkAndPlay('audio/car_drive_close.ogg');
    }
    // battle
    if(st >= ( 13 * win_h + 100 ) && st <= ( 13 * win_h + 422 ) ) {
        checkAndPlay('audio/battle.ogg');
    }
    // 滑落
    if(st >= ( 14.5 * win_h + 422 ) && st <= ( 15.5 * win_h + 422 ) ) {
        checkAndPlay('audio/slide_down.ogg');
    }
    // 追踪
    if(st >= ( 17.5 * win_h + 422 ) && st <= ( 18.5 * win_h + 422 ) ) {
        checkAndPlay('audio/bg_flying.ogg',true, true);
    }
    // 潜入
    if(st >= ( 21.7 * win_h + 422 ) && st <= ( 22.5 * win_h + 422 ) ) {
        checkAndPlay('audio/enter.ogg');
    }
    // 开灯
    if(st >= ( 25.5 * win_h + 422 ) && st <= ( 26 * win_h + 422 ) ) {
        checkAndPlay('audio/light_on.ogg');
    }
    // 结束音乐
    if(st >= ( 26 * win_h + 422 ) ) {
        checkAndPlay('audio/bg_end.ogg',true, true);
    }
}
